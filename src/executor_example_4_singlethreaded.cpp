/*
4.7   Two Nodes Running in Two Different Executors
As an extra example, see what happens if you use two Single-ThreadedExecutor
instances.
*/
...
/*
The odometry callback function works fine, but where is the log message that is
generated by the timer callback? As you can see, the Callback is not being
executed, and there is no error message.

This is because you can only have ONE SINGLE EXECUTOR per binary file.

Because of this, if you prefer to continue using SingleThreadedExecutors, your
only option is to create two different ROS2 programs, compiled as two
independent binary files.

As a side note:
In terms of performance, SingleThreadedExecutors are less CPU intensive than
MultiThreadedExecutors. So if it covers all your needs, it is preferable.
