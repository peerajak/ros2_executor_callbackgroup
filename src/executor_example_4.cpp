/*
4.6   Multi-Threaded Executor running two Nodes
*/
...
/*
You can see a TICK message every three seconds, and the odometry flows at a
continuous rate. This means that both Callbacks are working simultaneously in
parallel and not interfering with each other. Note: In this example, there is
one Callback per Node, so having one thread per Node works like a charm. You
will learn how to run one Node with multiple Callbacks later in this unit.
*/